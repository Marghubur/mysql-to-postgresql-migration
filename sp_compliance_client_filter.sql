CREATE OR REPLACE PROCEDURE public.sp_compliance_client_filter(
...
BEGIN
 ...
 _selectquery := concat('Select * from (
  ...
  c.*,
  Count(1) Over() as Total from compliance_client c
  Where ', _searchstring, '
...
EXCEPTION WHEN OTHERS THEN
  ...
  CALL public.sp_logexception(_message, '', 'sp_compliance_client_filter', 1::bit, 0::bit, _result);
END;
$$;
