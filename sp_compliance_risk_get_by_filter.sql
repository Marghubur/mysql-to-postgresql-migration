CREATE OR REPLACE FUNCTION public.sp_compliance_risk_get_by_filter(
...
BEGIN
 ...
 join compliance_file_detail fd
 on fd.fileid = jt.fileid
 where cr.clientid = _clientid
 and str_to_date(
 concat(cr.year, '-', lpad(cr.month,2,'0'), '-01'),
 '%Y-%m-%d'
...
EXCEPTION WHEN OTHERS THEN
  ...
  CALL public.sp_logexception(_message, '', 'sp_compliance_risk_get_by_filter', 1::bit, 0::bit, _result);
END;
$$;
